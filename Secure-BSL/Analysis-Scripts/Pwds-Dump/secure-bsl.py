...
class Memory:
    ...
    def updateNewPassword(self, ivtAdr, newPwd, userDefine, passphrase):
        updatePwd = []
        stringPwd = ''
        for i in newPwd:
    ...
    def securePassword(self, ivtAddress, data, userDefined, passphrase):
        ...
        for i in range(0,len(oldIVT), 2):
            ...
            addr = self.popRandomAddress()
            self.segments.append(Segment(addr, string.join(newISR,'')))
            newIVT.append(hex(addr))
        self.updateNewPassword(ivtAddress, newIVT, userDefined, passphrase)
    ...
    def loadIHex(self, file, userDefined, passphrase):
        ...
        if segmentdata:
            ...
            self.securePassword(startAddr, segmentdata, userDefined, passphrase)
