...
# Date: March 2016
# Description:  An improved bootstrap loader
# that better secures the BSL passwords.
##############################################  

import sys, time, string, cStringIO, struct, serial, os, glob, random, base64, hashlib
...
# Two Factor Using User Define Passphrase As Key to AES <Mauricio Tellez>
class AES256(object):
    def __init__(self, passphrase):
        self.bs = 32
        self.key = hashlib.sha256(passphrase.encode()).digest()

    def __pad__(self, str):
        return str + (self.bs - len(str) % self.bs) * chr(self.bs - len(str) % self.bs)

    def __unpad__(self, str):
        return str[:-ord(str[len(str)-1:])]

    def encrypt(self, text):
        raw = self.__pad__(text)
    ...
    def decrypt(self, text):
        ...
        cipher = AES.new(self.key, AES.MODE_CBC, iv)
        return self.__unpad__(cipher.decrypt(enc[AES.block_size:])).decode('utf-8')
...
class Memory:
    ...
    # Obtain a random unused address < Mauricio Tellez >
    def popRandomAddr(self):
        random.shuffle(self.unusedAdr)
        return self.unusedAdr.pop(0)
    # Generate a list of Unused addresses < Mauricio Tellez >
    def generateAddrRange(self):
        start = len(self.segments[0])+0x4000
        for i in range(start, 65502, 4):
            self.unusedAdr.append(i)

    # Update New Password (IVT) and output to password file <Mauricio Tellez>
    def securePassword(self, ivtAdr, newPwd, userDefine, passphrase):
        updatePwd = []
        stringPwd = ''
        for i in newPwd:
            updatePwd.append(chr(int(i[4:6],16)))
            updatePwd.append(chr(int(i[2:4],16)))
            stringPwd += " 0x" + i[4:6] + " 0x" + i[2:4]
	stringPwd += "\n"
        self.segments.append(Segment(ivtAdr, string.join(updatePwd,'')))
        try:
            os.remove("password")
        except OSError:
            pass
        with open("password","w") as fd:
            newPassword = stringPwd[1:]
            if userDefine != 0:
                pwdAES = AES256(passphrase)
                newPassword = pwdAES.encrypt(newPassword)
            fd.write(newPassword)

    # Improved BSL password for flash memory security <Mauricio Tellez>
    def generateRandomIVT(self, ivtAddress, data, userDefined, passphrase):
        self.generateAddrRange()
        oldIVT = data
        newIVT = []
        for i in range(0,len(oldIVT), 2):
            newISR = []
            newISR.append(chr(48)) # 0x30
            newISR.append(chr(64)) # 0x4
            newISR.append(oldIVT[i])
            newISR.append(oldIVT[i+1])
            addr = self.popRandomAddr()
            self.segments.append(Segment(addr, string.join(newISR,'')))
            newIVT.append(hex(addr))
        self.securePassword(ivtAddress, newIVT, userDefined, passphrase)


    def loadIHex(self, file, userDefined, passphrase):
        ...
        if segmentdata:
            print "Generating strong BSL password"
            self.generateRandomIVT(startAddr, segmentdata, userDefined, passphrase)
...
General options:
  ...
  -P, --password=file   Specify a file with the interrupt vectors that
                        ...
                        (e.g. -P INT_VECT.TXT).
  -t, --two-factor=pwd  User defined password to encrypte/decrypt the
                        password file. Default password is "password".
...
def main(itest=1):
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:],
            ...
            "hc:P:wf:m:eEpvrg:t:UDu:ds:xbITNB:S:V14",
            ["help", "comport=", "password=", "wait", "framesize=",
             "erasecycles=", "masserase", "erasecheck", "program",
             "verify", "reset", "go=", "two-factor=", "unpatched", "debug",
    ...
    for o, a in opts:
        if o in ("-h", "--help"):
            ...
            with open(a,'r') as fd:
                convertPwd = fd.read()
                dump = 1
        # Two Factor Support <Mauricio Tellez>
        elif o in ("-t","--two-factor"):
            userPwdUsed = 1
            passphrase = a
            if len(passphrase) < 8:
    ...
    if filetype is not None:                        #if the filetype is given...
        ...
        if filetype == 0:                           #select load function
            bsl.data.loadIHex(file,userPwdUsed,passphrase)                 #intel hex
        elif filetype == 1:
    ...
    # Decipher password if attempting to access flash memory <Mauricio Tellez>
    if dump == 1:
        temp = ''
        if userPwdUsed == 1:
	    try:
            	decryptPwd = AES256(passphrase)
            	convertPwd = decryptPwd.decrypt(convertPwd)
	    except UnicodeDecodeError:
		print "Incorrect passphrase"
		sys.exit()
        for i in convertPwd.split():
	    try:
                temp += chr(int(i,16))
	    except ValueError:
